  ...
  return (
    <nav className="flexBetween max-container padding-container relative z-30 py-3 text-white bg-[#2f4f4f]">
      ...
      <ul className=" h-full flex flex-row  lg:gap-12   md:gap-12 md:flexCenter md:flex  xs:gap-6 lg:flex ">
        {NAV_LINKS.map((link) => (
          <Link href={link.href} key={link.key} className="lg:text-[20px] text-white flexCenter cursor-pointer pb-1.5 transition-all hover:font-bold xs:text-[10px] xs:ml-1 xs:flex xs:flex-row  xl:text-[40px] ">
            {link.label}
          </Link>
        ))}
      </ul>
